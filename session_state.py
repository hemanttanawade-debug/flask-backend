...
3. SESSION PERSISTENCE:
   ...
   run_id) is written to uploads/session.json on every change.
...
7. RUN ID:
   - update_run_id(rid) stores the latest migration run_id from the frontend.
   - Persisted in session.json so it survives VM restarts.
   - Cleared only by hard_reset().
"""
...
def _load():
    ...
    """
    global session_id, config, csv_file_path, run_id, migration_active
...
def update_run_id(rid: str):
    """
    Store the latest migration run_id and persist to disk immediately.
    Survives VM restarts and gunicorn reloads.
    Cleared only by hard_reset().

    Example:
        state.update_run_id("abc123-run-id-from-frontend")
    """
    global run_id
    run_id = rid
    _persist()
...
def hard_reset():
    ...
    """
    global session_id, config, user_mappings, csv_file_path, run_id, migration_active
